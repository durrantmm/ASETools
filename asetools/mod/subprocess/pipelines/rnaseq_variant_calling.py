...
class RunRNASeqVariantCalling(RunSubprocessPipedSuper):
    ...
    if __name__ == '__main__':
        if __name__ == '__main__':
            def execute_steps(self):
                ...
                # Filter VCF for ASE
                # This uses my own script, vcf_filter_ase.py, to process the final VCF produced for the pipeline so that
                # It is ready to be analyzed by ASE.
                vcf_filter_for_ase_output_dir = join(self.output_dir, "STEP9_VCF_FILTER_FOR_ASE")
                vcf_filter_for_ase = RunVCFFilterASE(output_dir=vcf_filter_for_ase_output_dir,
                                                     input_vcf=filtered_vcf,
                                                     logger=self.logger)
                vcf_filter_for_ase.run()
